...
- **WHEN** an account manager submits a policy version and accepted scope with `ai_processing` set to boolean `true`
- **THEN** the service SHALL store consent evidence with the accepting identity and acceptance time

#### Scenario: Reject missing or false acceptance
- **WHEN** an account manager submits `ai_processing` as false or omits a boolean `true` acceptance
- **THEN** the service SHALL reject the request and SHALL NOT store consent evidence
