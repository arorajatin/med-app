...
- [ ] 3.4 Implement exact account-local assignment using NFKC, whitespace normalization, case folding, explicit aliases, and non-contradictory DOB; route every other result to `needs_assignment`.
...
- [ ] 5.3 Add normalization tests for all four trust classes, literal documented conditions, medications/labs/symptoms/general-association negative cases, missing literal values, semantically unsupported condition citations, fabricated or mismatched source references, atomic partial failure, retry supersession, and raw-output deletion.
- [ ] 5.4 Add assignment fixtures for exact names, explicit aliases, Unicode/whitespace/case variants, duplicate names, absent matches, contradictory DOB, and cross-account isolation.
