...
V1 accepts English-language, unencrypted PDF, JPEG, and PNG lab reports and prescriptions within the [product ceilings](specs/document-extraction/spec.md#requirement-restrict-v1-document-inputs). A valid but unsupported medical-document family remains privately stored with `unsupported_document_type` and publishes no derived data.

The [production provider design](../add-production-extraction-provider/design.md) owns the exact native-text gate, Textract/Bedrock configuration, source-layout conversion, staging cleanup, and region/ZDR preflight. It selects one method per logical document: `pdfplumber` only when every nonblank PDF page passes, otherwise whole-PDF Textract; images always use Textract. A single method keeps source coordinates consistent. Persist `native_text` or `textract_ocr` and the routing reason, then use the provider-neutral result in the assignment and review workflows above.

### Make retries attempt-aware and idempotent

One extraction job targets one complete immutable logical document. Every published observation and candidate retains an attempt identity. A retry stages a complete result set, atomically supersedes matching active observations without duplicates, and preserves prior review decisions where safe. Failed attempts leave committed output unchanged.

The [queue-worker design](../add-queue-backed-extraction-worker/design.md) owns claims, resumable provider phases, failure classification, attempt limits, jitter, callbacks, and recovery. Evaluate the provider's [quality gates](../add-production-extraction-provider/specs/document-extraction/spec.md#requirement-precision-gated-production-rollout) on approved held-out fixtures; production remains undeployed until those gates, provider privacy approval, and the Supabase boundary's [capability gates](../adopt-supabase-data-boundary/design.md#gate-rollout-by-capability) pass.
