...
- [ ] 1.1 Add immutable authenticated `direct_file`/`camera` logical-document manifest/version and ordered source-part persistence, plus extraction job generation, numbered attempt, persisted internal phase, claim token/lease, provider callback, active-result, safe-failure, transactional outbox, and cleanup/tombstone state.
